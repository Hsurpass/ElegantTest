...
static GSourceFuncs standardGLibSourceCallbackFuncs = {dispatchPrepare, dispatchCheck, dispatchExecute, NULL};
...
int main()
{
    ...
    std::shared_ptr<CommonAPI::MainLoopContext> mainloopContext =
        std::make_shared<CommonAPI::MainLoopContext>(connection);
    ...
    std::shared_ptr<E07MainloopProxy<>> myProxy =
        runtime->buildProxy<E07MainloopProxy>(domain, instance, mainloopContext);
