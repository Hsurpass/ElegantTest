...
public:
    client_sample(bool _use_tcp, bool _be_quiet, uint32_t _cycle)
        : app_(vsomeip::runtime::get()->create_application("Hello")),
          request_(vsomeip::runtime::get()->create_request(_use_tcp)), use_tcp_(_use_tcp), be_quiet_(_be_quiet),
          cycle_(_cycle), running_(true), blocked_(false), is_available_(false),
    ...
    bool init()
    {
        ...
        std::cout << "-----Client settings [protocol=" << (use_tcp_ ? "TCP" : "UDP")
                  << ":quiet=" << (be_quiet_ ? "true" : "false") << ":cycle=" << cycle_ << "]" << std::endl;
        ...
        app_->register_availability_handler(SAMPLE_SERVICE_ID + 1,
                                            SAMPLE_INSTANCE_ID,
                                            std::bind(&client_sample::on_availability,
                                                      this,
                                                      std::placeholders::_1,
    ...
    void stop()
    {
        ...
        std::cout << "------" << __func__ << " current-thread: " << std::this_thread::get_id()
                  << " offer-thread: " << sender_.get_id() << std::endl;
    ...
    {
        std::cout << "-----on_availability Service [" << std::setw(4) << std::setfill('0') << std::hex << _service
    ...
    void on_message(const std::shared_ptr< vsomeip::message >& _response)
    {
        ...
        std::cout << "-----on_message Received a response from Service [" << std::setfill('0') << std::hex
                  << std::setw(4) << _response->get_service() << "." << std::setw(4) << _response->get_instance()
                  << "] to Client/Session [" << std::setw(4) << _response->get_client() << "/" << std::setw(4)
                  << _response->get_session() << "]"
                  << ", response data: " << ss.str() << std::endl;
    ...
    void send()
    {
        std::cout << "-----" << __func__ << ", be_quiet_:" << be_quiet_ << std::endl;
        if (!be_quiet_) {
        ...
        while (running_) {
            {
                ...
                if (is_available_) {
                    app_->send(request_);
                    std::cout << "------Client/Session [" << std::setfill('0') << std::hex << std::setw(4)
                              << request_->get_client() << "/" << std::setw(4) << request_->get_session()
                              << "] sent a request to Service [" << std::setw(4) << request_->get_service() << "."
                              << std::setw(4) << request_->get_instance() << "]" << std::endl;
                    blocked_ = false;
                }
            }
