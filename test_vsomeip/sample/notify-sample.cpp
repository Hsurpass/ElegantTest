...
public:
    service_sample(uint32_t _cycle)
        : app_(vsomeip::runtime::get()->create_application("notify")), //
    ...
    bool init()
    {
        ...
        app_->offer_event(SAMPLE_SERVICE_ID,
                          SAMPLE_INSTANCE_ID,
                          SAMPLE_EVENT_ID,
                          its_groups,
                          vsomeip::event_type_e::ET_FIELD, /*field*/
    ...
    void run()
    {
        ...
        while (running_) {
            ...
            for (int i = 0; i < 10 && running_; i++)    // 超过10次，is_offer=false ,stop_offer
    ...
    void notify()
    {
        ...
        while (running_) {
            ...
            while (!is_offered_ && running_) {
                ...
                std::cout << "-----wait finish. its_lock status: " << its_lock.owns_lock() << std::endl;
            }
            std::cout << "-----run next. its_lock status: " << its_lock.owns_lock() << std::endl;

            while (is_offered_ && running_) {
                // std::cout << "-----sizeof(its_data): " << sizeof(its_data) << std::endl;
                ...
                std::cout << "-----notify_data_size: " << its_size;
                for (uint32_t i = 0; i < its_size; ++i) {
                    std::cout << its_data[i] << " ";
                }
                std::cout << std::endl;
