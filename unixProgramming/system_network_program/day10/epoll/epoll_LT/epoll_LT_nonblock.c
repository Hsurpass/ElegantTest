    ...
    while (1)
    {
        ...
        for (size_t i = 0; i < nready; i++)
        {
            ...
            if (epev[i].data.fd == listen_fd) // 监听文件描述符
            {
                ...
                // 设置为非阻塞IO
                int mod = fcntl(connfd, F_GETFL);
                mod |= O_NONBLOCK;
                fcntl(connfd, F_SETFL, mod);
            ...
            else // 其他套接字
            {
                ...
                while (ret = Read(sockfd, buf, sizeof(buf)))    // 非阻塞IO，轮询读
                {
                    if (ret == 0)
                    {
