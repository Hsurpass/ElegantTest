...
/*
kd-tree是一种用于数据检索的树形数据结构，用于kd-tree的数据可以是k维数据。kd-tree为二叉树结构，其每个节点都是k维数据点的超矩形区域。kd-tree一般用于多维空间关键数据的搜索和最近邻的查询。
其原理是将k维数据按照各维度排序，建立一棵二叉树。具体地，每次选择一个坐标轴进行划分，并按照此轴上各点的中位数将当前的超矩形划分为两个子区域，左子区域包含所有低于中位数的数据点，右子区域包含所有高于中位数的数据点。这个过程将递归地一直运行下去，直到区域中只有一个数据点时终止递归。
这样建立的kd-tree具有以下性质：
...
基于以上性质，我们可以使用kd-tree进行数据检索和最近邻的查询。例如，在进行查找时，我们可以通过比较查找点与分割平面的距离来确定从哪个子树开始搜索、可能更接近最近点的子树，从而达到快速查找的目的。
