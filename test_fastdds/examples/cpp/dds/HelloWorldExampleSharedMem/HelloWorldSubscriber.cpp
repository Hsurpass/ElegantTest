...
bool HelloWorldSubscriber::init()
{
    ...
    sm_transport->rtps_dump_file("rtps_dump.bin");// 跟踪subscriber流量

    // [OPTIONAL] ThreadSettings configuration
    // sm_transport->default_reception_threads(eprosima::fastdds::rtps::ThreadSettings{-1, 0, 0, -1});
    // sm_transport->set_thread_config_for_port(12345, eprosima::fastdds::rtps::ThreadSettings{-1, 0, 0, -1});
    // sm_transport->dump_thread(eprosima::fastdds::rtps::ThreadSettings{-1, 0, 0, -1});

    pqos.transport().user_transports.push_back(sm_transport);

    // Explicit configuration of SharedMem transport 服务发现使用共享内存
    // pqos.transport().use_builtin_transports = false;
...
void HelloWorldSubscriber::SubListener::on_data_available(DataReader* reader)
{
    ...
    if (reader->take_next_sample(hello_.get(), &info) == ReturnCode_t::RETCODE_OK) {
        if (info.valid_data) {
            ...
            std::cout << "Message " << hello_->message() << " " << hello_->index() << " RECEIVED With " << data_size
                      << "(bytes) of Data. DataEnd = " << (char*)&hello_->data()[data_size - 9] << std::endl;
        }
    }
}
