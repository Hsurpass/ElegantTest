...
LifespanSubscriber::LifespanSubscriber()
    : participant_(nullptr), subscriber_(nullptr), topic_(nullptr), reader_(nullptr), type_(new LifespanPubSubType())
{}
...
bool LifespanSubscriber::init(uint32_t lifespan_ms)
{
    ...
    rqos.lifespan().duration = lifespan_ms * 20;

    reader_ = subscriber_->create_datareader(topic_, rqos, &listener);
    if (reader_ == nullptr) {
        return false;
    }
    std::cout << __LINE__ << "reader: " << reader_ << std::endl;
...
void LifespanSubscriber::SubListener::on_subscription_matched(DataReader*, const SubscriptionMatchedStatus& info)
{
...
void LifespanSubscriber::SubListener::on_data_available(DataReader* reader)
{
    std::cout << __LINE__ << "reader: " << reader << std::endl;
...
void LifespanSubscriber::run(uint32_t number, uint32_t sleep_ms)
{
    ...
    for (uint32_t i = 0; i < listener.n_samples; i++) {
        ReturnCode_t errcode = reader_->take_next_sample((void*)&data, &info);
        // std::cout << "-----errcode: " << reinterpret_cast<uint32_t>(errcode) << std::endl;
