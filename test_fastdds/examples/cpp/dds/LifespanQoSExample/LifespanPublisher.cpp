...
LifespanPublisher::LifespanPublisher()
    : participant_(nullptr), publisher_(nullptr), topic_(nullptr), writer_(nullptr), type_(new LifespanPubSubType())
{}
...
bool LifespanPublisher::init(uint32_t lifespan_ms)
{
    ...
    //Create DataWriter
    DataWriterQos wqos;
    wqos.history().kind = KEEP_ALL_HISTORY_QOS; // 要想发送历史数据还必须启用reliability、durability
    // wqos.history().kind = KEEP_LAST_HISTORY_QOS;
    wqos.history().depth = 30; // 只有设置为KEEP_LAST_HISTORY_QOS，depth才有效
    wqos.reliability().kind = RELIABLE_RELIABILITY_QOS;
    wqos.durability().kind = TRANSIENT_LOCAL_DURABILITY_QOS;
    // wqos.lifespan().duration = lifespan_ms * 1e-3;
    wqos.lifespan().duration = lifespan_ms * 5;
