...
struct Arg : public option::Arg
{
    ...
    static option::ArgStatus NumericRange(const option::Option& option, bool msg)
    {
        ...
        if (option.arg != nullptr) {
            ...
            if (endptr != option.arg && *endptr == 0 && value >= min && value <= max) {
                return option::ARG_OK;
            }
        }
        ...
        if (msg) {
            ...
            os << "' requires a numeric argument in range [" << min << ", " << max << "]" << std::endl;
...
const option::Descriptor usage[] = {
    {UNKNOWN_OPT, 0, "", "", Arg::None, "Usage: HelloWorldExample <publisher|subscriber>\n\nGeneral options:"},
