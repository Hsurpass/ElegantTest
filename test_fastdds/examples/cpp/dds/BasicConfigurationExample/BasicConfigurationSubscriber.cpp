...
bool HelloWorldSubscriber::init(const std::string& topic_name,
...
void HelloWorldSubscriber::SubListener::set_max_messages(uint32_t max_messages)
{
...
void HelloWorldSubscriber::run(uint32_t samples)
{
    ...
    if (samples > 0) {
        std::cout << "Subscriber running until " << samples
                  << " samples have been received. Please press CTRL+C to stop the Subscriber at any time."
                  << std::endl;
    }
    else {
